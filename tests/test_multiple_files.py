...
def test_name_clash_overwrite_without_ftp(
    singlepartserver, multipartserver, sftp_server, tmp_path
):
    dm = Downloader(overwrite=True)
    ...
    assert set([file.name for file in files]) == set(
        ["testfile.txt", "testfile.1.txt", "multipartfile.txt"]
    )
